...
function wordToRgb(color: string): string | null {
  const colors = {
    red: 'rgba(255, 0, 0, 1)',
...
function strToArry(color: string, reg: RegExp): Array<number> {
  return color
    .replace(reg, '')
    .split(/\s|,/g)
    .filter((v) => v !== '')
...
function checkIsColor(color: string): string | null {
  if (/^#/.test(color)) {
    return 'hex';
...
function limit(v: string | number, max: string | number) {
  v = typeof v === 'string' && v.indexOf('.') !== -1 && parseFloat(v) === 1 ? '100%' : v;
  v = Math.min(max as number, Math.max(0, parseFloat(`${v}`)));
  v = typeof v === 'string' && (v as string).indexOf('%') !== -1 ? parseInt(`${v * (max as number)}`, 10) / 100 : v;
  return Math.abs(v - (max as number)) < 0.000001 ? 1 : (v % (max as number)) / parseFloat(max as string);
}

function rgbToHsv(r: number | string, g: number | string, b: number | string): {
...
class Color {
  ...
  _hsla(hsla: string, fromHsv: (_h: number, _s: number, _v: number) => void): void {
    const hslaArry = strToArry(hsla, /hsla|hsl|\(|\)/gm);
    if (hslaArry.length === 4) {
      this._a = Math.floor(parseFloat(hslaArry[3] as unknown as string) * 100);
  ...
  _rgba(rgba: string, fromHsv: (_h: number, _s: number, _v: number) => void): void {
    const rgbaArry = strToArry(rgba, /rgba|rgb|\(|\)/gm);
    if (rgbaArry.length === 4) {
      this._a = Math.floor(parseFloat(rgbaArry[3] as unknown as string) * 100);
  ...
  _hex(hex: string, fromHsv: (_h: number, _s: number, _v: number) => void): void {
